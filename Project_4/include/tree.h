...
private:
    ...
    Node **children; //pointer to four other nodes
    int width; //当前像素区块的宽度
...
public:
    ...
    Node *create_tree(PNG *p, int width, int height, int x, int y);

    bool is_repeat(Node *node) const;

    void judge_for_leaf(Node *node, int threshold);

    void judging(Node *node, int R, int G, int B);
};
